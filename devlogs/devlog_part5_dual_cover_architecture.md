---
title: 一张图搞定两种比例：MD2WX 双封面拼接引擎与微信草稿箱坐标直推实践
author: 野生宝藏箱
digest: 微信公众号图文接口只允许关联一张封面素材，但在订阅号大图（2.35:1）和聊天卡片（1:1）下存在截然不同的比例需求。本文记录 MD2WX 如何通过 3350x1000 左右横向拼接与归一化裁剪坐标直推草稿箱，兼顾两种构图需求；同时梳理近期代码块真机净化和 Cloudflare R2 图床换链的底层设计。
...
# 一张图搞定两种比例：MD2WX 双封面拼接引擎与微信草稿箱坐标直推实践

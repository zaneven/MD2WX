...
6. **开发日志与技术文章编写规范（必须严格遵守）**：
   - **文章标题字数限制**：文章主标题（frontmatter 的 `title` 与一级标题 `H1`）**严禁超过 20 字**。
   - **摘要（digest）字数限制**：Frontmatter 中的 `digest` 简明精炼，**严格控制在 50 字以内**。
   - **严禁在 Frontmatter 填写 cover**：写开发日志或功能推广文时，**一律不要填写 `cover` 字段**（留空或直接省略）。
   - **遵循去 AI 味准则（humanizer-zh）**：语言平实、自然、克制、讲人话，严禁公关宣发修辞、多余感叹号与空洞排比。
